...
% CASO 1: Encontramos el vehículo objetivo (Aquí usamos el mismo ID)
moveVehicle(
    [vehicle(ID, Or, R, C, L) | RV],
    ID, Steps, [FV | RV]) :-
    
    movement(Or, R, C, Steps, NewR, NewC) ,
    FV = vehicle(ID, Or, NewR, NewC, L), !. 

% CASO 2: NO es el vehículo objetivo (Usamos variables DISTINTAS)
moveVehicle([vehicle(CocheActualID, Or, R, C, L) | RV],
        TargetID, Steps, [vehicle(CocheActualID, Or, R, C, L) | NewState]) :-
    
    % Verificamos que sean diferentes
    CocheActualID \= TargetID,

    % Seguimos buscando recursivamente
    moveVehicle(RV, TargetID, Steps, NewState).
...
% primero debemos normalizar la entrada

% Traduce la lista completa del tablero
normalizar_tablero([], []).
normalizar_tablero([H|T], [HNorm|TNorm]) :-
    normalizar(H, HNorm),
    normalizar_tablero(T, TNorm).
...
% Caso 1: El vehículo 0 está en la cabeza y toca la salida (Col + Len = 6)
es_solucion([vehicle(0, _, _, C, L) | _]) :-
    Limit is C + L,
    Limit >= 6, !.

% Caso 2: El vehículo 0 no es la cabeza, buscamos en el resto
es_solucion([_ | Resto]) :-
    es_solucion(Resto).

% usamos el reverse eficiente -> con acumulador
reverse(MovimientosRev, Movimientos) :-
    reverseAcc(MovimientosRev, [], Movimientos) .

% vamos llenando un acumulador paulatinamente
reverseAcc([], Acc, Acc).
reverseAcc([M | RV], Acc , NuevosMovimientos) :-
    reverseAcc(RV, [M | Acc], NuevosMovimientos).
...
recolectar_movimientos_id(ID, Tablero, MovsPrevios, ListaHijos) :-
    ...
    try_step(ID, -4, Tablero, MovsPrevios, L7, ListaInvertida),

    reverse(ListaInvertida, ListaHijos).
